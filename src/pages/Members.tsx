...
const btnGhost =
  'h-[32px] px-4 rounded-sm border border-border bg-surface text-[14px] text-text-secondary cursor-pointer hover:bg-surface-hover transition-colors';
const btnPrimary =
  'btn-primary h-[38px] px-5 text-[13.5px] font-semibold cursor-pointer disabled:cursor-not-allowed';
const btnDanger =
  'h-[32px] px-4 rounded-sm bg-danger text-[14px] font-medium text-white cursor-pointer hover:opacity-90 transition-opacity';
...
function Field({ label, required, children }: { label: string; required?: boolean; children: ReactNode }) {
  return (
    <label className="block">
      <span className="block text-[13px] text-text-secondary mb-[6px]">
        {label}
...
export default function Members() {
  ...
  const cards: Metric[] = [
    ...
    { icon: UserCog, value: countOf('待激活'), label: '待激活', hint: '已邀请，尚未完成首次登录', tone: countOf('待激活') ? 'attention' : 'neutral' },
  ...
  function submitDisable() {
    ...
    dispatch({ type: 'SET_MEMBER_STATUS', memberId: disableTarget.id, status: '已停用' });
  ...
  const actions = canManage ? (
    <button
      type="button"
      onClick={openInvite}
      className="btn-primary flex items-center gap-[6px] h-[38px] px-5 text-[13.5px] font-semibold cursor-pointer"
    >
      <UserPlus size={15} />
      邀请成员
    </button>
  ...
  return (
    <div>
                        ...
                        <td colSpan={columns.length} className="px-5 py-3">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {seats.map((a) => (
                              ...
                              >
                                {moduleOf(state, a.moduleId)?.name ?? '未知模块'}
                              </span>
                            ))}
                          </div>
                        </td>
      ...
      <Modal open={disableTarget !== null} onClose={() => setDisableTarget(null)} title="停用成员" width={500}>
        {disableTarget && (
          <>
            ...
            <div className="mt-5 flex items-center justify-end gap-2">
              <button type="button" className={btnGhost} onClick={() => setDisableTarget(null)}>
                取消
              </button>
