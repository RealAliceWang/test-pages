      ...
      <div className="p-6 flex flex-col gap-5">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => handleTypeSwitch('trial')}
            className={`h-[40px] px-5 rounded-[--radius-md] text-[14px] font-medium inline-flex items-center gap-2 cursor-pointer transition-all ${
        ...
        {recordType === 'trial' ? (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
            {([
              ...
              <div key={i} className="relative rounded-[--radius-lg] px-5 py-5 overflow-hidden"
                ...
                <div className="relative flex items-center gap-4">
                  <div className="w-[44px] h-[44px] rounded-[--radius-lg] flex items-center justify-center shrink-0" style={{ background: c.iconBg, color: c.accent }}>{c.icon}</div>
                  <div>
        ...
        {/* Filters + search */}
        <div className="bg-surface rounded-[--radius-md] px-5 py-3 flex items-center justify-between">
          {recordType === 'trial' ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="h-[32px] px-3 text-[14px] font-medium text-text-secondary bg-surface-hover rounded-[--radius-sm] cursor-pointer inline-flex items-center gap-[6px] hover:bg-border transition-colors">
              <Download size={14} /> 导出
            </button>
          </div>
        </div>
        ...
        {detail && (
          <div className="flex flex-col gap-4 text-[14px]">
            ...
            <div className="flex justify-end pt-1">
              <button onClick={() => setDetail(null)} className="h-[36px] px-4 text-[14px] font-medium text-white bg-primary rounded-[--radius-sm] cursor-pointer hover:bg-primary-dark transition-colors">关闭</button>
            </div>
          </div>
        )}
      </Modal>
      ...
      <Modal open={!!purchaseDetail} onClose={() => setPurchaseDetail(null)} title="订单详情">
        {purchaseDetail && (
          <div className="flex flex-col gap-4 text-[14px]">
            ...
            <div className="flex justify-end pt-1">
              <button onClick={() => setPurchaseDetail(null)} className="h-[36px] px-4 text-[14px] font-medium text-white bg-primary rounded-[--radius-sm] cursor-pointer hover:bg-primary-dark transition-colors">关闭</button>
            </div>
          </div>
        )}
      </Modal>
