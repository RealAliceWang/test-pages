...
系统带完整的会话闭环：账号菜单底部可**退出登录**，`/login` 提供账号密码登录（登录卡底部有一行低调的演示账号文字链，可一键换角色），`/register` 走 B 端语义的「申请加入企业」——注册后账号为**待激活**，登录会被拦截，需企业管理员在成员管理中激活（注册动作同时写入审计日志）。演示痕迹刻意收敛，界面按正式产品呈现。演示会话默认已登录为张思远，也可以直接用右上角账号菜单里的身份切换在四个角色之间切换：
